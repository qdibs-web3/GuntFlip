...
const CoinFlipPage = () => {
  ...
  const getWalletClient = useCallback(() => {
    if (!account || !account.provider) {
      console.error("Wallet account or provider not available from AGW.");
      setError("Wallet provider not available. Please ensure your AGW wallet is connected and configured correctly.");
      return null;
    }
    // AGW's account.provider should be an EIP-1193 compatible provider
    return createWalletClient({
      account: account.address, // AGW provides the address directly
      chain: abstractTestnetChain,
      transport: custom(account.provider), // Use the provider from AGW
    });
  }, [account]);
